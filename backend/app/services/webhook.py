...
class WebhookService:
    ...
    async def send_webhook(self, url: str, payload: Dict[str, Any], secret: str) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0, follow_redirects=False) as client:
                ...
                try:
                    webhook_logger.info(f"Sending POST webhook request to {clean_url} with json parameter")
                    webhook_logger.info(f"Payload preview: {str(payload)[:200]}...")
                    response = await client.post(
                        clean_url,
                        json=payload,  # Let httpx handle serialization
                        headers=headers
                    )
                    webhook_logger.info(f"Webhook response received: {response.status_code} {response.reason_phrase}")
                    ...
                    try:
                        ...
                        webhook_logger.info(f"Sending POST webhook request to {clean_url} with manual serialization")
                        response = await client.post(
                            clean_url,
    ...
    async def create_webhook_event(self, db: Session, user_id: uuid.UUID, import_job_id: uuid.UUID, 
        ...
        # Ensure we're using the enum value properly
        if isinstance(event_type, str):
            webhook_logger.warning(f"Received string event_type: {event_type}, should be WebhookEventType enum")
            # Try to convert string to enum if needed
            try:
                event_type = WebhookEventType(event_type)
            except ValueError:
                webhook_logger.error(f"Invalid event type string: {event_type}")
                return None
        ...
        try:
            ...
            webhook_logger.info(f"Webhook payload keys: {list(payload.keys()) if isinstance(payload, dict) else 'Not a dict'}")
